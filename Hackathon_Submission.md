...
3. **False Positive Reduction:** The dual-model approach (Isolation Forest baseline filtering + XGBoost strict classification) achieves a verified ~39.6% precision rate on the strict Top-1% Alert Budget (Tier 1), while capturing stealthier, high-complexity attacks via the Tier 2 safety net (Chain Detection & High Confidence Classification), resulting in an overall combined queue precision of ~74.7%.
