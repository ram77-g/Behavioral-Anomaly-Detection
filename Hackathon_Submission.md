...
* **Deep Learning Sequence Modeling (PyTorch GRU):** A stateful GRU neural network (seeded deterministically for mathematical reproducibility) that processes chronological user events. It predicts the exact next resource and auth method, outputting a prediction loss (`ae_error`) to mathematically quantify chronological deviation.
...
* **Supervised Attack Classification:** An **XGBoost** model (using Stratified Train/Test splits and sample weighting) evaluates the anomalies (including the new `ae_error` 8th feature). It outputs a probability distribution across all attack classes, selecting the highest probability as the prediction and the raw percentage as the **AI Confidence Level**.
...
* **Live Simulation Engine (FastAPI):** A background `asyncio` engine running inside FastAPI that streams events, computes rolling baselines continuously in real-time, and scores them dynamically rather than replaying static scores. It utilizes a stateful RAM buffer (`entity_state_buffers`) to maintain a 9-event sliding chronological window per-entity for real-time PyTorch inference. The synthetic traffic generation operates completely unseeded, ensuring a truly dynamic, unpredictable live feed.
